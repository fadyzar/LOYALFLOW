/*
  # עדכון אוטומטי של נתוני נאמנות בעת השלמת תור

  1. שינויים
    - הוספת פונקציה לעדכון נתוני נאמנות
    - הוספת טריגר שמופעל בעת שינוי סטטוס תור ל"הושלם"
    - עדכון אוטומטי של נקודות ויהלומים לפי הגדרות הנאמנות של העסק
  
  2. אבטחה
    - הפונקציה מוגדרת כ-SECURITY DEFINER
    - בדיקות תקינות לפני עדכון הנתונים
*/
...
BEGIN
  ...
  IF (NEW.status = 'completed' AND OLD.status != 'completed') THEN
    ...
    IF (v_consecutive_visits >= v_consecutive_visits_required) THEN
      v_diamonds := v_diamonds + v_diamonds_per_consecutive_visits;
    ...
    INSERT INTO appointment_logs (
      ...
      jsonb_build_object(
        ...
        'diamonds_added', CASE WHEN v_consecutive_visits = 0 THEN v_diamonds_per_consecutive_visits ELSE 0 END,
