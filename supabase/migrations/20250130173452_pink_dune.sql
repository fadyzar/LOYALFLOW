...
BEGIN
  ...
  INSERT INTO public.users (
    ...
    END,
    CASE 
      WHEN user_role = 'staff' AND NEW.raw_user_meta_data ? 'specialties' THEN
  ...
  -- For admin users, create new business and related data
  IF user_role = 'admin' AND business_id IS NULL THEN
    ...
    SET 
      business_id = business_id,
      metadata = jsonb_set(metadata, '{business_id}', to_jsonb(business_id))
    WHERE id = NEW.id;
